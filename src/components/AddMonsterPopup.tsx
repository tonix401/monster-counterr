...
import MonsterSuggestionInput from './MonsterSuggestionInput';
...
const AddMonsterPopup: React.FC<AddMonsterPopupProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent, nextField: 'hp' | 'amount' | 'add') => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (nextField === 'add') {
        handleAdd();
      } else {
        const fieldId = nextField === 'hp' ? 'hp-input' : 'amount-input';
  ...
  return (
    <Popup isOpen={isOpen} onClose={onClose} title="Add Monster" width={300}>
