...
const MonsterInfoPopup: React.FC<MonsterInfoPopupProps> = ({
    isOpen,
    onClose,
    monsterDetails,
}) => {
    if (!monsterDetails) {
        return (
            <Popup
                isOpen={isOpen}
                onClose={onClose}
                title="Monster details not found"
                width={400}
            >
    ...
    const createAttributeRow = (
        ...
        return (
            <div
                className={`attribute-row ${column ? "attribute-column" : ""}`}
                key={attrName}
            >
        ...
        <Popup isOpen={isOpen} onClose={onClose} width={1100}>
            ...
            <div className="monster-name-picture-area">
                <div className="monster-name-area">
                    ...
                    <span>
                        {monsterDetails.size} {monsterDetails.type},{" "}
            ...
            {monsterDetails.proficiencies.length > 0 &&
                createAttributeRow(
                    ...
                    monsterDetails.proficiencies
                        .map(
                            (prof) => `${prof.proficiency.name}: +${prof.value}`
                        )
                        .join(", ")
                )}
            ...
            {createAttributeRow(
                ...
                Object.entries(monsterDetails.senses)
                    .map(
                        ([sense, val]) => `${sense.replace(/_/g, " ")}: ${val}`
                    )
                    .join(", ")
            )}
