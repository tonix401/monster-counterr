...
import { createInfoSlice } from './slices/infoSlice';
import { createXpSlice } from './slices/xpSlice';
import { createDataManagementSlice } from './slices/dataManagementSlice';
import { temporal, type TemporalState, type TemporalActions } from './middleware/temporal';
    ...
    temporal(
      ...
      killAllMonsters: () => {
        ...
        monsters.forEach((monster: Monster, index: number): void => {
          const delay: number = length > 5 ? Math.round((index * ANIMATION_DURATION.KILL_ALL_DELAY) / length) : 0;
          setTimeout((): void => {
